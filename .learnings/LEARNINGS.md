...
**Logged**: 2026-05-11T06:15:00Z
**Priority**: medium
**Status**: resolved
**Area**: frontend

### Summary
贡献热力图注释里写的贡献总数（175）与实际 REAL_CONTRIB_DATA 求和结果（167）不一致，人工写入的注释数字写错了。

### Details
`// Total: 175 contributions` 这条注释是手动估算后写入的，但实际 JSON 数据求和只有 167。workflow 代码里正确地用 `Object.values(contribData).reduce(...)` 计算了 `contribTotal` 并替换了这个注释，但本地测试数据是手动写的，所以注释写错了。

**解决方案**：手动检查 REAL_CONTRIB_DATA 的实际求和结果，修正注释为 `// Total: 167 contributions`。

### Metadata
- Source: test_suite
- Related Files: index.html
- Tags: data-integrity, manual-data
- Pattern-Key: frontend.contrib_comment_mismatch

---

## [LRN-20260511-007] correction
...
### Summary
hero-tags 静态 HTML 中包含 Java 标签，但 LANG_STATS（来自 GitHub API）中并没有任何 Java 项目，数据不一致。

### Details
`index.html` 第 856 行的 hero-tags 静态 HTML 包含 `Java` 标签（橙色），但 LANG_STATS 数据中没有 Java 语言（用户的仓库没有 Java 项目）。这导致页面展示的技能标签与实际语言分布数据不一致，是一个维护性 bug。

**解决方案**：将 Java 标签替换为 CSS 标签（因为 CSS 在 LANG_STATS 中排名第5）。

### Metadata
- Source: test_suite
- Related Files: index.html
- Tags: data-inconsistency, static-html
- Pattern-Key: frontend.herometa_tags_mismatch

---

## [LRN-20260511-008] best_practice
...
### Summary
数组访问时使用 `|| ''` 作为默认值不够精确，可能掩盖数组越界问题。

### Details
在贡献热力图的 tooltip 生成代码中：
```javascript
const dateIdx = wi * 7 + di - (startDow > 0 ? startDow : 0);
cell.title = `${dates[dateIdx] || ''}: ${day.count} contribution...`;
```
`dates[dateIdx]` 当越界时返回 `undefined`，`undefined || ''` 变成空字符串，tooltip 变成 `: 75 contributions`。这种方式在调试时无法发现问题。

**更好的做法**：使用 `dates[dateIdx] !== undefined` 做显式检查，或使用 `dates[dateIdx] ?? '???'`（nullish coalescing）。

### Metadata
- Source: test_suite
- Related Files: index.html
- Tags: javascript, array-bounds, null-safety
- Pattern-Key: frontend.array_access_guard

---

## [LRN-20260511-009] correction
...
### Summary
neofetch 命令中的 Memory 数值使用了旧的硬编码（7168 MB / 32768 MB），GitHub Actions runner 的实际规格已更新。

### Details
neofetch 中显示 `Memory: 7168 MB / 32768 MB`，但当前 GitHub hosted runner 实际规格约为 `8192 MB / 16384 MB`（4核 × 2线程 = 7GB，标准 SAzure VMs）。虽然 runner 规格会变化，但作为固定展示数据应该反映当前更常见的新版 runner 规格。

**解决方案**：更新为 `Memory: 8192 MB / 16384 MB (typical)` 并标注为典型值。

### Metadata
- Source: test_suite
- Related Files: index.html
- Tags: hardcoded-data, accuracy
- Pattern-Key: frontend.neofetch_memory

---

## [LRN-20260511-010] knowledge_gap
...
### Summary
workflow 的 regex 在修复后仍然依赖于旧格式的 HTML 注释文本，如果将来改注释格式，regex 会再次失效。

### Details
之前的 regex 问题（`// Real language data from GitHub API` 前的缩进、`// Featured Projects data` 没有 `from GitHub API`）都是因为 regex 和实际 HTML 格式不匹配。虽然当前已经修复了 regex 使其更健壮（动态检测缩进），但如果将来开发者改了 HTML 中的注释文本，workflow regex 仍然会失效，导致数据不再更新。

**建议**：在 workflow 的 commit message 或注释中加入版本标记，或者在 workflow 中先验证 regex 是否匹配（assert-match），若不匹配则输出警告并拒绝提交。

### Metadata
- Source: investigation
- Related Files: .github/workflows/update-data.yml, index.html
- Tags: workflow, regex, maintenance
- Pattern-Key: infra.workflow_regex_brittleness
...
### Summary
综合测试套件（100个测试）是发现隐藏 bug 的最有效手段，尤其擅长发现数据不一致和逻辑错误。

### Details
本轮测试套件（100个测试）发现了4个实际问题：
1. 贡献注释数字写错（175 vs 167）
2. hero-tags 有 Java 但 LANG_STATS 没有 Java
3. 数组越界时 tooltip 用空字符串掩盖
4. neofetch 内存值过时

这些 bug 都是代码审查或人工测试很难发现的，因为它们不导致报错，只是展示错误的数据。自动化测试套件通过比对多个数据源（PROJECTS vs LANG_STATS、注释 vs 实际求和）成功发现了这些问题。

**建议**：为 portfolio 建立 CI 测试，在每次 push 时运行 `node test-suite.js` 确保数据一致性。

### Metadata
- Source: test_suite
- Related Files: test-suite.js
- Tags: testing, automation, data-integrity
- Pattern-Key: testing.bug_finding_value

---

## [LRN-20260511-012] correction

**Logged**: 2026-05-11T06:30:00Z
**Priority**: critical
**Status**: resolved (code fixed, awaiting deploy)
**Area**: infra

### Summary
Cloudflare Worker 访客计数器缺少会话去重逻辑，导致同一设备的多次刷新都被计入计数器，桌面端和移动端看到完全不同的数字。
...
// 每次 POST 都无条件 +1，无任何去重！
```

**后果**：
- 桌面浏览器刷新10次 = Worker累计10 = 显示"第10位访客"
- 移动浏览器刷新10次 = Worker累计20 = 显示"第20位访客"
- 同一设备每次刷新都+1，数字快速膨胀
- 桌面端和移动端显示的数字完全不同

**解决方案**：在 Worker 端增加 IP+UA 的 30 分钟 TTL 去重：
```javascript
...
if (lastSeen !== null) {
  // 30分钟内已访问过，只返回当前计数，不+1
  const counter = await env.WAYHHOW_VISITORS.get('counter') || '0';
  return jsonResp({ count: parseInt(counter, 10) });
}
// 新访客：+1 并记录会话
counter++;
await env.WAYHHOW_VISITORS.put(seenKey, String(Date.now()), { expirationTtl: 1800 });
```

**注意**：Worker 源码在 `visitor-counter-worker/index.js`，但部署需要单独执行 `wrangler deploy`。

### Metadata
- Source: root_cause_analysis
- Related Files: visitor-counter-worker/index.js
- Tags: cloudflare-workers, kv, session-dedup, visitor-counter
- Pattern-Key: infra.worker_no_session_dedup
